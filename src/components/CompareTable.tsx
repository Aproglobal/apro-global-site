...
export default function CompareTable() {
  ...
  const headers = [
    'Model','Model No.','Guidance','Seats','Variant','Deck','Reverse','Max speed','Grade','Battery','Motor','Dimensions (L×W×H)','Payload',
  ];

  return (
    <section
      id="compare"
      className="py-16 bg-zinc-50 text-black border-t border-zinc-200 dark:bg-zinc-900 dark:text-white dark:border-zinc-800 scroll-mt-24 md:scroll-mt-28"
    >
      <div className="max-w-6xl mx-auto px-5 relative">
            ...
            <thead>
              <tr>
                {headers.map((h) => (
                  <th
                    key={h}
                    className="py-3 px-3 text-left text-zinc-600 dark:text-zinc-300 sticky top-14 md:top-16 bg-zinc-50 dark:bg-zinc-900 z-10"
                  >
                    {h}
                  </th>
                ))}
            ...
            <tbody>
              {MODELS.map((m, idx) => {
                ...
                return (
                  <tr
                    key={m.code}
                    className={`border-t border-zinc-200 dark:border-zinc-800 hover:bg-zinc-100 dark:hover:bg-zinc-800 cursor-pointer ${idx % 2 ? 'bg-white/50 dark:bg-transparent' : ''}`}
                    tabIndex={0}
                    role="button"
                    ...
                    onMouseEnter={(e) =>
                      setTip({ show: true, x: e.clientX, y: e.clientY, code: m.code, content: tipContent, viaKeyboard: false })
                    }
                    onMouseMove={(e) => setTip((t) => (t.show ? { ...t, x: e.clientX, y: e.clientY } : t))}
                    onMouseLeave={() => setTip({ show: false, x: 0, y: 0 })}
                    onFocus={(e) => {
                      const rect = (e.currentTarget as HTMLElement).getBoundingClientRect();
                      setTip({ show: true, x: rect.right - 12, y: rect.top + rect.height / 2, code: m.code, content: tipContent, viaKeyboard: true });
                    }}
        ...
        {tip.show && (
          <div
            className="pointer-events-none fixed z-30 max-w-[80vw] md:max-w-md px-3 py-2 rounded-lg shadow-lg text-xs md:text-sm leading-relaxed bg-white text-black dark:bg-zinc-800 dark:text-white border border-zinc-200 dark:border-zinc-700"
            style={{
        ...
        <div className="mt-6">
          <a
            ...
            className="inline-block px-5 py-3 rounded-lg font-medium bg-black text-white hover:opacity-90 dark:bg-white dark:text-black whitespace-nowrap"
          >
