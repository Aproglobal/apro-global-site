...
type Props = {
  items: FeatureItem[];
  /** 저해상도(≈300×300) 이미지일 때 켜면, 중앙 작은 썸네일 + 블러 백그라운드 보정 */
  lowRes?: boolean;
...
export default function TechFeatureGrid({ items, lowRes = false, thumbMaxPx = 180 }: Props) {
  ...
  return (
    <section aria-labelledby="tech-feature-grid-title" className="py-10">
      <h3 id="tech-feature-grid-title" className="sr-only">
        Technology Features
      </h3>
      ...
      <ul className="grid gap-4 grid-cols-2 sm:grid-cols-3 lg:grid-cols-4">
        {display.map((f, i) => (
          <li
            key={f.id}
            className="
              group relative rounded-2xl overflow-hidden border border-zinc-200 dark:border-zinc-800 
          ...
          >
            <div className="relative aspect-square">
              ...
              <div className="absolute inset-x-0 bottom-0 p-3 sm:p-4">
                ...
                <h4 className="mt-1 text-sm sm:text-base font-semibold">{f.title}</h4>
                {f.note && <p className="mt-1 text-xs text-zinc-600 dark:text-zinc-300 line-clamp-2">{f.note}</p>}
              </div>
