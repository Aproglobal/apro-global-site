        ...
        >
          ...
          <div className="grid md:grid-cols-2">
            ...
            <div className="relative bg-white dark:bg-zinc-900 grid place-items-center p-2 md:p-3">
              <link rel="preload" as="image" href={imgs[0]} />
              ...
              <div className="flex justify-center gap-2 py-2 md:py-0 md:absolute md:bottom-3 md:left-1/2 md:-translate-x-1/2">
                {imgs.map((_, i) => (
                  <button
                    ...
                    className={["w-2.5 h-2.5 rounded-full transition", i === imgIdx ? "bg-white" : "bg-white/50 hover:bg-white/70"].join(" ")}
                  />
                ))}
              </div>
            </div>
            ...
            <div className="p-4 sm:p-6">
              <p className="text-sm text-zinc-600 dark:text-zinc-300">
                {model.guidance} • {model.seats} seats {model.deck ? `• ${model.deck} deck` : ""} {model.variant ? `• ${model.variant}` : ""}{" "}
                {model.reverse ? "• Reverse Seating" : ""}
              </p>
                ...
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {ORDER.map((key) => {
                      ...
                      const value = spec?.[key] ?? (key === "reverseSeating" ? (model.reverse ? "Yes" : "No") : undefined);
              ...
              <div className="mt-6 flex flex-wrap gap-3">
                ...
                <a href={specHref} onClick={() => trackEvent("spec_download", { code: model.code })} className="px-5 py-3 rounded-full border border-zinc-300 dark:border-zinc-700">
                  Download specs (PDF)
                </a>
              </div>
