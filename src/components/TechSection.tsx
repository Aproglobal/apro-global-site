...
export default function TechSection({ copy }: { copy: TechCopy }) {
  // 이미지 피처 목록 (데이터 파일만 교체해도 동작)
  const items: TechItem[] = useMemo(() => TECH_FEATURES, []);

  const [view, setView] = useState<ViewMode>("gallery"); // 모바일에서만 사용
          ...
          <div className="max-w-3xl w-full" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="mt-3 flex items-start justify-between gap-4 text-white">
              <div>
                ...
                {(active.oneLiner ?? active.desc) && (
                  <p className="text-sm text-zinc-300 mt-1">{active.oneLiner ?? active.desc}</p>
                )}
              </div>
              <button
                type="button"
